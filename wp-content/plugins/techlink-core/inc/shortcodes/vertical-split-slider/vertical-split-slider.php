...
if ( ! function_exists( 'techlink_core_add_vertical_split_slider_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function techlink_core_add_vertical_split_slider_shortcode( $shortcodes ) {
		$shortcodes[] = 'TechLinkVerticalSplitSliderShortcode';

		return $shortcodes;
	}

	add_filter( 'techlink_core_filter_register_shortcodes', 'techlink_core_add_vertical_split_slider_shortcode' );
}

if ( class_exists( 'TechLinkCoreShortcode' ) ) {
	class TechLinkVerticalSplitSliderShortcode extends TechLinkCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( TECHLINK_CORE_SHORTCODES_URL_PATH . '/vertical-split-slider' );
			$this->set_base( 'techlink_vertical_split_slider' );
			$this->set_name( esc_html__( 'Vertical Split Slider', 'techlink-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds vertical split slider holder', 'techlink-core' ) );
			...
			$this->set_scripts(
				array(
					...
					'multiscroll' => array(
						'registered'	=> false,
						'url'			=> TECHLINK_CORE_SHORTCODES_URL_PATH . '/vertical-split-slider/assets/js/plugins/jquery.multiscroll.min.js',
			...
			$this->set_necessary_styles(
				array(
					'multiscroll' => array(
						'registered'	=> false,
						'url'			=> TECHLINK_CORE_SHORTCODES_URL_PATH . '/vertical-split-slider/assets/css/plugins/jquery.multiscroll.css'
					)
				)
			);
			...
			$this->set_option( array(
				'field_type' => 'repeater',
				'name'       => 'children',
				'title'      => esc_html__( 'Slide Items', 'techlink-core' ),
				'items'      => array(
					array(
						'field_type' => 'select',
						'name'       => 'slide_header_style',
						'title'      => esc_html__( 'Header/Bullets Style', 'techlink-core' ),
						'options'    => array(
					...
					array(
						'field_type' => 'select',
						'name'       => 'slide_layout',
						'title'      => esc_html__( 'Slide Layout', 'techlink-core' ),
						'options'    => array(
							'image-left'  => esc_html__( 'Image On Left', 'techlink-core' ),
							'image-right' => esc_html__( 'Image On Right', 'techlink-core' )
						),
					),
					array(
						'field_type' => 'image',
					...
					array(
						...
						'title'      => esc_html__( 'Content Image', 'techlink-core' )
					),
					array(
						'field_type' => 'text',
		...
		public function render( $options, $content = null ) {
			...
			return techlink_core_get_template_part( 'shortcodes/vertical-split-slider', 'templates/vertical-split-slider', '', $atts );
		}
