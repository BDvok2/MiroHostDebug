...
if ( ! function_exists( 'techlink_core_register_breadcrumbs_title_layout' ) ) {
	/**
	 ...
	 * @return array
	 */
	function techlink_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'TechLinkCoreBreadcrumbsTitle';
		
		return $layouts;
	}
	
	add_filter( 'techlink_core_filter_register_title_layouts', 'techlink_core_register_breadcrumbs_title_layout');
}

if ( ! function_exists( 'techlink_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function techlink_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'techlink-core' );
		
		return $layouts;
	}
	
	add_filter( 'techlink_core_filter_title_layout_options', 'techlink_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'techlink_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function techlink_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();
		
		// Breadcrumbs label
		$labels = apply_filters( 'techlink_core_filter_breadcrumbs_label', array(
			'home'        => esc_html__( 'Home', 'techlink-core' ),
			'tag'         => esc_html__( 'Posts tagged "%s"', 'techlink-core' ),
			'author'      => esc_html__( 'Posted by %s', 'techlink-core' ),
			'search'      => esc_html__( 'Search results for "%s"', 'techlink-core' ),
			'404'         => esc_html__( '404 - Page not found', 'techlink-core' ),
			'query_paged' => esc_html__( '(Page %s)', 'techlink-core' )
		) );
		
		// Breadcrumbs variables
		$settings = apply_filters( 'techlink_core_filter_breadcrumbs_settings', array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'techlink_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}
		
		echo apply_filters( 'techlink_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}