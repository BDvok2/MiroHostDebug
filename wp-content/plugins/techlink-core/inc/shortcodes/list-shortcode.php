...
abstract class TechLinkCoreListShortcode extends QodeFrameworkShortcode {
	private $post_type;
	...
	public function map_list_options( $params = array() ) {
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'behavior', $exclude_behavior ) ) ) {
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'behavior',
				'title'         => esc_html__( 'List Appearance', 'techlink-core' ),
				'options'       => techlink_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'masonry', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Image Proportions', 'techlink-core' ),
				'options'       => array(
					''      => esc_html__( 'Original', 'techlink-core' ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'images_proportion', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'default_value' => 'full',
				'title'         => esc_html__( 'Image Proportions', 'techlink-core' ),
				'options'       => techlink_core_get_select_type_options_pool( 'list_image_dimension', false ),
				'group'         => $group,
				'dependency'    => array(
					'show' => array(
						'behavior' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Width', 'techlink-core' ),
				'description' => esc_html__( 'Enter image width in px', 'techlink-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Height', 'techlink-core' ),
				'description' => esc_html__( 'Enter image height in px', 'techlink-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
						'images_proportion' => array(
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'columns', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'options'       => techlink_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
				'default_value' => '3',
				'group'         => $group,
				'dependency'    => array(
					'hide' => array(
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Columns Responsive', 'techlink-core' ),
				'options'       => techlink_core_get_select_type_options_pool( 'columns_responsive' ),
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'columns_1440',
				'title'         => esc_html__( 'Number of Columns 1367px - 1440px', 'techlink-core' ),
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'columns_1366',
				'title'         => esc_html__( 'Number of Columns 1025px - 1366px', 'techlink-core' ),
				'options'       => techlink_core_get_select_type_options_pool( 'columns_number' ),
				'default_value' => '3',
				'dependency'    => array(
					'show' => array(
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'columns_1024',
				'title'         => esc_html__( 'Number of Columns 769px - 1024px', 'techlink-core' ),
				'options'       => techlink_core_get_select_type_options_pool( 'columns_number' ),
				'default_value' => '3',
				'dependency'    => array(
					'show' => array(
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'columns_768',
				'title'         => esc_html__( 'Number of Columns 681px - 768px', 'techlink-core' ),
				'options'       => techlink_core_get_select_type_options_pool( 'columns_number' ),
				'default_value' => '3',
				'dependency'    => array(
					'show' => array(
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'columns_680',
				'title'         => esc_html__( 'Number of Columns 481px - 680px', 'techlink-core' ),
				'options'       => techlink_core_get_select_type_options_pool( 'columns_number' ),
				'default_value' => '3',
				'dependency'    => array(
					'show' => array(
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'columns_480',
				'title'         => esc_html__( 'Number of Columns 0 - 480px', 'techlink-core' ),
				'options'       => techlink_core_get_select_type_options_pool( 'columns_number' ),
				'default_value' => '3',
				'dependency'    => array(
					'show' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Row Height', 'techlink-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Max Row Height', 'techlink-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Last Row Treshold', 'techlink-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type' => 'select',
			'name'       => 'slider_loop',
			'title'      => esc_html__( 'Enable Slider Loop', 'techlink-core' ),
		...
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Enable Slider Autoplay', 'techlink-core' ),
			'options'    => techlink_core_get_select_type_options_pool( 'yes_no' ),
			'dependency' => $dependency,
			'group'      => $group
		) );
		$this->set_option( array(
			'field_type' => 'select',
			'name'       => 'slider_center',
			'title'      => esc_html__( 'Center Slider', 'techlink-core' ),
		...
		$this->set_option( array(
			'field_type' => 'select',
			'name'       => 'slider_width',
			'title'      => esc_html__( 'Enable Split Slider', 'techlink-core' ),
		...
		$this->set_option( array(
			'field_type'  => 'text',
			'name'        => 'slider_speed',
			'title'       => esc_html__( 'Slide Duration', 'techlink-core' ),
			'description' => esc_html__( 'Default value is 5000 (ms)', 'techlink-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
			...
			'title'       => esc_html__( 'Slide Animation Duration', 'techlink-core' ),
			'description' => esc_html__( 'Speed of slide animation in milliseconds. Default value is 800.', 'techlink-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Enable Slider Navigation', 'techlink-core' ),
			'options'    => techlink_core_get_select_type_options_pool( 'yes_no' ),
			'dependency' => $dependency,
			'group'      => $group
		) );
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Enable Slider Pagination', 'techlink-core' ),
			'options'    => techlink_core_get_select_type_options_pool( 'yes_no' ),
			'dependency' => $dependency,
			'group'      => $group
		) );
	...
	public function get_list_classes( $atts ) {
		...
		if ( isset( $atts['slider_width'] ) && $atts['slider_width'] === 'yes' ) {
			$holder_classes[] = 'qodef-slider-width--display-center-content';
		}
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && $atts['behavior'] == 'masonry' && ! empty( $atts['masonry_images_proportion'] ) && $atts['masonry_images_proportion'] == 'fixed' ) {
			...
			$image_dimension              = techlink_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}
	...
	public function map_query_options( $params = array() ) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'techlink-core' );
		...
		$this->set_option( array(
			'field_type'    => 'text',
			'name'          => 'posts_per_page',
			'title'         => esc_html__( 'Posts per Page', 'techlink-core' ),
			'default_value' => '-1',
			'group'         => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'orderby',
			'title'         => esc_html__( 'Order By', 'techlink-core' ),
			'options'       => techlink_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
			'default_value' => 'date',
			'group'         => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'order',
			'title'         => esc_html__( 'Order', 'techlink-core' ),
			'options'       => techlink_core_get_select_type_options_pool( 'order' ),
			'default_value' => 'DESC',
			'group'         => $group
		) );
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'additional_params', $exclude_option ) ) ) {
			...
			do_action( 'techlink_core_action_map_query_options_before_additional', $group );

			$additional_params = apply_filters( 'techlink_core_filter_map_additional_query_params', array(
				''       => esc_html__( 'No', 'techlink-core' ),
				'id'     => esc_html__( 'Post IDs', 'techlink-core' ),
				'tax'    => esc_html__( 'Taxonomy Slug', 'techlink-core' ),
				'author' => esc_html__( 'Author Name', 'techlink-core' ),
			) );
			...
			$this->set_option( array(
				'field_type'  => 'text',
				'name'        => 'post_ids',
				'title'       => esc_html__( 'Posts IDs', 'techlink-core' ),
				'description' => esc_html__( 'Separate post IDs with commas', 'techlink-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
			...
			if ( ! empty( $taxonomies_formatted ) ) {
				...
				$this->set_option( array(
					'field_type' => 'text',
					'name'       => 'tax_slug',
					'title'      => esc_html__( 'Taxonomy Slug', 'techlink-core' ),
					'group'      => $group,
					'dependency' => array(
						'show' => array(
							'additional_params' => array(
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'author_slug',
				'title'      => esc_html__( 'Author Slug', 'techlink-core' ),
				'group'      => $group,
				'dependency' => array(
					'show' => array(
						'additional_params' => array(
			...
			do_action( 'techlink_core_action_map_query_options_after_additional', $group );
		}
	}
	...
	public function get_additional_query_args( $atts ) {
		...
		$args = apply_filters( 'techlink_core_filter_additional_query_args', $args, $atts, $this->get_post_type() );
	...
	public function map_layout_options( $params = array() ) {
		...
		$this->set_option( array(
			...
			'name'          => 'layout',
			'title'         => esc_html__( 'Item Layout', 'techlink-core' ),
			'options'       => $layouts,
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'text_transform', $exclude_option ) ) ) {
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'text_transform',
				'title'      => esc_html__( 'Title Text Transform', 'techlink-core' ),
	...
	public function map_additional_options( $params = array() ) {
		$group_name         = esc_html__( 'Additional', 'techlink-core' );
		...
		if ( ! $exclude_filter ) {
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'enable_filter',
				'title'         => esc_html__( 'Enable Filter', 'techlink-core' ),
				'description'   => esc_html__( 'Enabling this option will show categories filter above list', 'techlink-core' ),
			...
			do_action( 'techlink_core_action_map_additional_options_after_filter', $group_name );
		}
		...
		if ( ! $exclude_pagination ) {
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Pagination', 'techlink-core' ),
				'options'       => techlink_core_get_select_type_options_pool( 'pagination_type' ),
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Load More Top Margin (px or %)', 'techlink-core' ),
				'default_value' => '',
				'group'         => $group_name,
				'dependency'    => array(
					'show' => array(
			...
			do_action( 'techlink_core_action_map_additional_options_after_pagination', $group_name );
		}
	}
	...
	public function register_list_scripts() {
		...
		$list_scripts = apply_filters( 'techlink_core_filter_register_list_shortcode_scripts', isset( $scripts ) ? $scripts : array() );

		$this->set_scripts( $list_scripts );
	}

	public function load_assets() {
		do_action( 'techlink_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}
