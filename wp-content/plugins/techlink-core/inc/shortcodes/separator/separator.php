...
if ( ! function_exists( 'techlink_core_add_separator_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function techlink_core_add_separator_shortcode( $shortcodes ) {
		$shortcodes[] = 'TechLinkCoreSeparatorShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'techlink_core_filter_register_shortcodes', 'techlink_core_add_separator_shortcode', 9 );
}

if ( class_exists( 'TechLinkCoreShortcode' ) ) {
	class TechLinkCoreSeparatorShortcode extends TechLinkCoreShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( TECHLINK_CORE_SHORTCODES_URL_PATH . '/separator' );
			$this->set_base( 'techlink_core_separator' );
			$this->set_name( esc_html__( 'Separator', 'techlink-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays separator with provided parameters', 'techlink-core' ) );
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'position',
				'title'      => esc_html__( 'Position', 'techlink-core' ),
				'options'    => array(
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'border_style',
				'title'      => esc_html__( 'Border Style', 'techlink-core' ),
				'options'    => techlink_core_get_select_type_options_pool( 'border_style' ),
				'group'      => esc_html__( 'Style', 'techlink-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'width',
				'title'      => esc_html__( 'Width (px or %)', 'techlink-core' ),
				'group'      => esc_html__( 'Style', 'techlink-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'thickness',
				'title'      => esc_html__( 'Thickness (px)', 'techlink-core' ),
				'group'      => esc_html__( 'Style', 'techlink-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin_top',
				'title'      => esc_html__( 'Margin Top (px or %)', 'techlink-core' ),
				'group'      => esc_html__( 'Style', 'techlink-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin_bottom',
				'title'      => esc_html__( 'Margin Bottom (px or %)', 'techlink-core' ),
				'group'      => esc_html__( 'Style', 'techlink-core' )
			) );
		}
		...
		public function render( $options, $content = null ) {
			...
			return techlink_core_get_template_part( 'shortcodes/separator', 'templates/separator', '', $atts );
		}
