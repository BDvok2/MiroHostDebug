<?php
$is_enabled = techlink_core_get_post_value_through_levels( 'qodef_blog_single_enable_author_info' );

if ( $is_enabled === 'yes' && get_the_author_meta( 'description' ) !== '' ) {
	$author_id     = get_the_author_meta('ID');
	$author_link   = get_author_posts_url( $author_id );
	$email_enabled = techlink_core_get_post_value_through_levels( 'qodef_blog_single_enable_author_info_email' ) === 'yes';
	$user_socials  = techlink_core_get_author_social_networks( $author_id );
	?>
		...
		<div class="qodef-m-inner">
			...
			<div class="qodef-m-content">
				<h5 class="qodef-m-author vcard author">
					<span class="qodef-m-author-label"><?php esc_html_e( 'The author', 'techlink-core' ); ?></span>
					<a itemprop="url" href="<?php echo esc_url( $author_link ); ?>">
					...
					<div class="qodef-m-social-icons">
						<?php foreach ( $user_socials as $social ) { ?>
							<a itemprop="url" class="<?php echo esc_attr( $social['class'] ) ?>" href="<?php echo esc_url( $social['url'] ) ?>" target="_blank">
								<?php echo qode_framework_icons()->render_icon( $social['icon'], 'simple-line-icons' ); ?>
							</a>
						<?php } ?>
					</div>
				<?php } ?>
