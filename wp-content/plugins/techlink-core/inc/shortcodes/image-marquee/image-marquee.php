...
if ( ! function_exists( 'techlink_core_add_image_marquee_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function techlink_core_add_image_marquee_shortcode( $shortcodes ) {
		$shortcodes[] = 'TechLinkCoreImageMarqueeShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'techlink_core_filter_register_shortcodes', 'techlink_core_add_image_marquee_shortcode' );
}

if ( class_exists( 'TechLinkCoreShortcode' ) ) {
	class TechLinkCoreImageMarqueeShortcode extends TechLinkCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'techlink_core_filter_image_marquee_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'techlink_core_filter_image_marquee_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( TECHLINK_CORE_SHORTCODES_URL_PATH . '/image-marquee' );
			$this->set_base( 'techlink_core_image_marquee' );
			$this->set_name( esc_html__( 'Image Marquee', 'techlink-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Image Marquee element', 'techlink-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'duration',
				'title'         => esc_html__( 'Animation Duration (Seconds)', 'techlink-core' ),
				'default_value' => '20',
			) );
			$this->set_option( array(
				'field_type' => 'select',
		...
		public function render( $options, $content = null ) {
			...
			return techlink_core_get_template_part( 'shortcodes/image-marquee', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
