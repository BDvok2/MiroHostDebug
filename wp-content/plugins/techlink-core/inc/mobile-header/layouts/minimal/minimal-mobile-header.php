...
class MinimalMobileHeader extends TechLinkCoreMobileHeader {
	...
	public function __construct() {
		...
		add_action( 'techlink_core_after_mobile_header_logo_image', array( $this, 'set_logo_image' ) );
	...
	function fullscreen_menu_template() {
		$header = techlink_core_get_post_value_through_levels( 'qodef_header_layout' );
	...
	function set_logo_image() {
		...
		$logo_dark_image_id  = techlink_core_get_post_value_through_levels( 'qodef_logo_dark' );
		$logo_light_image_id = techlink_core_get_post_value_through_levels( 'qodef_logo_light' );
		...
		if ( ! empty( $logo_dark_image_id ) ) {
			$logo_dark_image_attr = array(
				...
				'itemprop' => 'image',
				'alt'      => esc_attr__( 'logo dark', 'techlink-core' )
			);
		...
		if ( ! empty( $logo_light_image_id ) ) {
			$logo_light_image_attr = array(
				...
				'itemprop' => 'image',
				'alt'      => esc_attr__( 'logo light', 'techlink-core' )
			);
		...
		if ( ! empty( $logo_dark_image_id ) || ! empty( $logo_light_image_id ) ) {
			techlink_core_template_part( 'mobile-header/layouts/minimal/templates', 'parts/mobile-logo-light-dark', '', $parameters );
