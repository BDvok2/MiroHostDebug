...
class TechLinkCoreTopArea {
	...
	public function __construct() {
		...
		add_action( 'techlink_action_before_page_header', array( $this, 'load_template' ) );
		...
		add_filter( 'techlink_filter_add_inline_style', array( $this, 'set_inline_top_area_styles' ) );
		add_filter( 'techlink_core_filter_top_area_inner_class', array( $this, 'set_grid_class' ) );
	}
	...
	function is_top_area_enabled() {
		$top_area_enabled = techlink_core_get_post_value_through_levels( 'qodef_top_area_header' ) === 'yes';
	...
	public function set_top_area_height() {
		if ( $this->is_top_area_enabled ) {
			$height         = techlink_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
	...
	public function is_top_area_transparent() {
		$background_color = techlink_core_get_post_value_through_levels( 'qodef_top_area_header_background_color' );
	...
	public function load_template() {
		...
		techlink_core_template_part( 'header/top-area/', 'templates/top-area', '', $parameters );
	}
	...
	public function set_inline_top_area_styles( $style ) {
		...
		$height       = techlink_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
		$side_padding = techlink_core_get_post_value_through_levels( 'qodef_top_area_header_side_padding' );
	...
	public function set_grid_class( $class ) {
		$class .= techlink_core_get_post_value_through_levels( 'qodef_top_area_header_in_grid' ) == 'yes' ? 'qodef-content-grid' : '';
		
		return $class;
	}
}

TechLinkCoreTopArea::get_instance();
