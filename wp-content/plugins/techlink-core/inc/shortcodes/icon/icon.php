...
if ( ! function_exists( 'techlink_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function techlink_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'TechLinkCoreIconShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'techlink_core_filter_register_shortcodes', 'techlink_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'TechLinkCoreShortcode' ) ) {
	class TechLinkCoreIconShortcode extends TechLinkCoreShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( TECHLINK_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'techlink_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'techlink-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'techlink-core' ) );
			...
			$this->set_option( array(
				'field_type'	=> 'iconpack',
				'name'			=> 'main_icon',
				'title'			=> esc_html__( 'Icon', 'techlink-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'size',
				...
				'options'    => array(
					...
					'tiny'    => esc_html__( 'Tiny', 'techlink-core' ),
					'small'   => esc_html__( 'Small', 'techlink-core' ),
					'medium'  => esc_html__( 'Medium', 'techlink-core' ),
					'large'   => esc_html__( 'Large', 'techlink-core' ),
					'huge'    => esc_html__( 'Huge', 'techlink-core' ),
				)
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'custom_size',
				'title'      => esc_html__( 'Custom Size', 'techlink-core' ),
			) );
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'icon_layout',
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'border_radius',
				'title'      => esc_html__( 'Border Radius', 'techlink-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
							'values'        => 'square',
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'shape_size',
				'title'      => esc_html__( 'Shape Size', 'techlink-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'border_width',
				'title'      => esc_html__( 'Border Width', 'techlink-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
							'values'        => array( 'circle', 'square' ),
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'techlink_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return techlink_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
