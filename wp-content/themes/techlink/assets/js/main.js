(function ( $ ) {
	...
	var qodefModifiedButton = {
		init: function () {
			var $button = $('.woocommerce-form-login__submit, .coupon button, button[name="update_cart"], .checkout-button, #place_order, .wc-forward, .woocommerce-Button, .wc-backward, .qodef-woo-product-button-holder a, .single_add_to_cart_button, .woocommerce-form-coupon button');
			...
			if (button.length) {
				button.each(function () {
					...
					if ( thisButton.find('.qodef-btn-masked').length === 0 ){
						thisButton.html('<span class="qodef-m-text">'+ buttonText +'</span>');
					}
				});
			}
		}
	...
	var qodefFloat = {
		init: function () {
			var $items = $('.qodef-custom-float-section');
			var $decorations = $('.qodef-content-line-decoration > svg');
			...
			if ( $decorations.length ) {
				$decorations.each(function () {
					var $thisDecoration = $(this);

					$thisDecoration.attr('data-parallax', '{"y": -450, "smoothness": 25}');
				});
			}

			setTimeout( function () {
				qodefFloat.initFloat();
			}, 100);
		},
		initFloat: function () {
