<?php
$is_enabled = techlink_core_get_post_value_through_levels( 'qodef_portfolio_enable_navigation' );

if ( $is_enabled === 'yes' ) {
	$through_same_category = techlink_core_get_post_value_through_levels( 'qodef_portfolio_navigation_through_same_category' ) === 'yes';
	?>
	<div id="qodef-single-portfolio-navigation" class="qodef-m">
		<div class="qodef-m-inner">
			...
			$post_navigation = array(
				'prev'      => array(
					'label' => '<span class="qodef-m-nav-label">' . esc_html__( 'Previous Page', 'techlink-core' ) . '</span>',
					'icon'  => '<span class="qodef-m-nav-icon-holder">' . techlink_get_svg_icon( 'slider-arrow-left', 'qodef-m-nav-icon' ) . '</span>'
				),
				'back-link' => array(),
				'next'      => array(
					'label' => '<span class="qodef-m-nav-label">' . esc_html__( 'Next Page', 'techlink-core' ) . '</span>',
					'icon'  => '<span class="qodef-m-nav-icon-holder">' . techlink_get_svg_icon( 'slider-arrow-right', 'qodef-m-nav-icon' ) . '</span>'
				)
			);
			...
			if ( $back_to_link !== '' ) {
				$post_navigation['back-link'] = array(
					...
					'icon'    => '<span class="qodef-m-nav-icon-holder">' . techlink_get_svg_icon( 'back-to-home', 'qodef-m-nav-icon' ) . '</span>'
				);
			}
				...
				if ( isset( $post_navigation[ $key ]['post'] ) ) {
					...
					<a itemprop="url" class="qodef-m-nav qodef--<?php echo esc_attr( $key ); ?>" href="<?php echo esc_url( get_permalink( $post_id ) ); ?>">
						<?php if ( ! empty( $value['icon'] ) ) {
							echo qode_framework_wp_kses_html( 'html', $value['icon'] );
						} ?>
