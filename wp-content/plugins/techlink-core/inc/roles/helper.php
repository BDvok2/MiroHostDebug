...
if ( ! function_exists( 'techlink_core_include_role_custom_fields' ) ) {
	/**
	 * Function that includes role custom fields files
	 */
	function techlink_core_include_role_custom_fields() {
		foreach ( glob( TECHLINK_CORE_INC_PATH . '/roles/*/role-fields.php' ) as $role_fields ) {
			include_once $role_fields;
		}
	}
	
	add_action( 'qode_framework_action_custom_user_fields', 'techlink_core_include_role_custom_fields' );
}

if ( ! function_exists( 'techlink_core_register_role_custom_fields' ) ) {
	/**
	 * Function that registers role custom fields files
	 */
	function techlink_core_register_role_custom_fields() {
		do_action( 'techlink_core_action_register_role_custom_fields' );
	}
	
	add_action( 'qode_framework_action_custom_user_fields', 'techlink_core_register_role_custom_fields', 11 );
}

if ( ! function_exists( 'techlink_core_profile_get_social_params' ) ) {
	/**
	 * Returns owner params
	 */
	function techlink_core_profile_get_social_params( $params ) {
		...
		foreach ( $params['user']->roles as $role ) {
			if ( in_array( $role, $roles_social_scope ) ) {
				$enable = true;
				break;
			}
		}
	...
	add_filter( 'techlink_membership_filter_user_params', 'techlink_core_profile_get_social_params' );
}

if ( ! function_exists( 'techlink_core_social_edit_profile_fields' ) ) {
	function techlink_core_social_edit_profile_fields( $page, $params ) {
		$roles_social_scope = apply_filters( 'techlink_core_filter_role_social_array', array( 'administrator', 'author' ));
		$enable = false;
	...
	add_action( 'techlink_membership_action_after_dashboard_edit_profile_fields', 'techlink_core_social_edit_profile_fields', 15, 2 );
}

if ( ! function_exists( 'techlink_core_user_profile_social_update' ) ) {
	function techlink_core_user_profile_social_update( $options, $user_id ) {
	...
	add_action( 'techlink_membership_action_update_user_profile', 'techlink_core_user_profile_social_update', 10, 2 );
}