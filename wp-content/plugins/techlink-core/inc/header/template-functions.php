...
if ( ! function_exists( 'techlink_core_get_header_logo_image' ) ) {
	/**
	 * This function print header logo image
	 *
	 * @param array $parameters
	 */
	function techlink_core_get_header_logo_image( $parameters = array() ) {
		$header_skin     = techlink_core_get_post_value_through_levels( 'qodef_header_skin' );
		$logo_height     = techlink_core_get_post_value_through_levels( 'qodef_logo_height' );
		$customizer_logo = techlink_core_get_customizer_logo();
		...
		foreach ( $available_logos as $logo ) {
			...
			$logo_image_id = techlink_core_get_post_value_through_levels( 'qodef_logo_' . $logo );
			
			if ( empty( $logo_image_id ) && ! empty( $header_skin ) ) {
				$logo_image_id = techlink_core_get_post_value_through_levels( 'qodef_logo_main' );
			}
			...
			if ( ! empty( $logo_image_id ) ) {
				$logo_image_attr = array(
					...
					'itemprop' => 'image',
					'alt'      => sprintf( esc_attr__( 'logo %s', 'techlink-core' ), $logo )
				);
		...
		if ( $is_enabled ) {
			echo apply_filters( 'techlink_core_filter_get_header_logo_image', techlink_core_get_template_part( 'header/templates', 'parts/logo', '', $parameters ), $parameters );
...
if ( ! function_exists( 'techlink_core_get_header_widget_area' ) ) {
	/**
	 ...
	 */
	function techlink_core_get_header_widget_area( $header_layout = '', $widget_area = 'one' ) {
		$page_id    = qode_framework_get_page_id();
		$is_enabled = get_post_meta( $page_id, 'qodef_show_header_widget_areas', true ) !== 'no';

		if ( $is_enabled ) {
			$parameters  = apply_filters( 'techlink_core_filter_header_widget_area', array(
				'page_id'             => $page_id,
			...
			techlink_core_template_part( 'header/templates', 'parts/widgets', '', $parameters );
		}
	}
}