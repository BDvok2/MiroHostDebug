...
if ( ! function_exists( 'techlink_core_add_image_gallery_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function techlink_core_add_image_gallery_shortcode( $shortcodes ) {
		$shortcodes[] = 'TechLinkCoreImageGalleryShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'techlink_core_filter_register_shortcodes', 'techlink_core_add_image_gallery_shortcode' );
}

if ( class_exists( 'TechLinkCoreListShortcode' ) ) {
	class TechLinkCoreImageGalleryShortcode extends TechLinkCoreListShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( TECHLINK_CORE_SHORTCODES_URL_PATH . '/image-gallery' );
			$this->set_base( 'techlink_core_image_gallery' );
			$this->set_name( esc_html__( 'Image Gallery', 'techlink-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image gallery element', 'techlink-core' ) );
			...
			$this->set_option( array(
				...
				'name'       => 'images',
				'multiple'   => 'yes',
				'title'      => esc_html__( 'Images', 'techlink-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'image_size',
			...
			$this->map_list_options( array(
				...
				'group'            => esc_html__( 'Gallery Settings', 'techlink-core' )
			) );
		}

		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'techlink_core_image_gallery', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return techlink_core_get_template_part( 'shortcodes/image-gallery', 'templates/image-gallery', $atts['behavior'], $atts );
		}
