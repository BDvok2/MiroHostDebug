...
if ( ! function_exists( 'techlink_core_add_pricing_table_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function techlink_core_add_pricing_table_shortcode( $shortcodes ) {
		$shortcodes[] = 'TechLinkCorePricingTableShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'techlink_core_filter_register_shortcodes', 'techlink_core_add_pricing_table_shortcode' );
}

if ( class_exists( 'TechLinkCoreShortcode' ) ) {
	class TechLinkCorePricingTableShortcode extends TechLinkCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'techlink_core_filter_pricing_table_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'techlink_core_filter_pricing_table_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( TECHLINK_CORE_SHORTCODES_URL_PATH . '/pricing-table' );
			$this->set_base( 'techlink_core_pricing_table' );
			$this->set_name( esc_html__( 'Pricing Table', 'techlink-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds pricing table element', 'techlink-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'featured_table',
				'title'         => esc_html__( 'Featured Table', 'techlink-core' ),
			...
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'subtitle_color',
				'title'      => esc_html__( 'Subitle Color', 'techlink-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'price',
				'title'      => esc_html__( 'Price', 'techlink-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'price_color',
				'title'      => esc_html__( 'Price Color', 'techlink-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'currency',
				'title'      => esc_html__( 'Currency', 'techlink-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'currency_color',
				'title'      => esc_html__( 'Currency Color', 'techlink-core' )
			) );
			$this->set_option( array(
				'field_type' => 'html',
				'name'       => 'content',
				'title'      => esc_html__( 'Content', 'techlink-core' )
			) );
			$this->import_shortcode_options( array(
				'shortcode_base'    => 'techlink_core_button',
				'exclude'           => array( 'custom_class', 'button_layout', 'size', 'border_color', 'hover_border_color', 'margin', 'padding' ),
		...
		public function render( $options, $content = null ) {
			...
			return techlink_core_get_template_part( 'shortcodes/pricing-table', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function generate_button_params( $atts ) {
			$atts['button_layout'] = 'additional';
			$atts['size']          = 'full';

			$params = $this->populate_imported_shortcode_atts( array(
				'shortcode_base' => 'techlink_core_button',
				'exclude'        => array( 'custom_class', 'border_color', 'hover_border_color', 'margin', 'padding' ),
				'atts'           => $atts,
			) );

			$params['custom_class']  = 'qodef-no-border-radius';
