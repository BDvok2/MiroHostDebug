...
if ( ! function_exists( 'techlink_core_add_wishlist_profile_navigation_item' ) ) {
	function techlink_core_add_wishlist_profile_navigation_item( $items, $dashboard_url ) {
		$user = wp_get_current_user();

		$items['my-wishlist'] = array(
			'url'         => esc_url( add_query_arg( array( 'user-action' => 'my-wishlist' ), $dashboard_url ) ),
			'text'        => esc_html__( 'My Wishlist', 'techlink-core' ),
	...
	add_filter( 'techlink_membership_filter_dashboard_navigation_pages', 'techlink_core_add_wishlist_profile_navigation_item', 10, 2 );
}

if ( ! function_exists( 'techlink_core_add_wishlist_profile_navigation_pages' ) ) {
	function techlink_core_add_wishlist_profile_navigation_pages( $html, $action ) {

		if ( 'my-wishlist' === $action ) {
			$atts = array();

			$html = techlink_core_get_template_part( 'wishlist', '/profile/templates/my-wishlist', '', $atts );
		}

		return $html;
	}

	add_filter( 'techlink_membership_filter_dashboard_page', 'techlink_core_add_wishlist_profile_navigation_pages', 10, 2 );
}
